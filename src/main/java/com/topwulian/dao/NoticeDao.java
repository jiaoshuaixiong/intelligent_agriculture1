...
public interface NoticeDao {
	...
	@Select("select id,title,status,createTime,updateTime from t_notice order by createTime desc limit #{offset},#{rows}")
    List<Notice> recentlyNotice(@Param("offset") Integer offset, @Param("rows") Integer rows);
}
